...
def build_rows(timeseries, metadata):
    ...
    for point_idx in range(len(timeseries["points"])):
        ...
        metric = {"type": timeseries["metric"]["type"]}
        ...
        resource = {"type": timeseries["resource"]["type"]}
        ...
        point = {"interval": interval, "value": value_type_label}
        row["point"] = point
        ...
        if "metadata" in timeseries:
            ...
            if "systemLabels" in timeseries["metadata"]:
                system_labels_list = get_system_labels(
...
def build_distribution_value(value_json):
    ...
    if "linearBuckets" in value_json["bucketOptions"]:
        linearBuckets = {
            "numFiniteBuckets": value_json["bucketOptions"]["linearBuckets"][
                "numFiniteBuckets"
            ],
        ...
        exponentialBuckets = {
            "numFiniteBuckets": value_json["bucketOptions"]["exponentialBuckets"][
                "numFiniteBuckets"
            ],
            "growthFactor": round(
                value_json["bucketOptions"]["exponentialBuckets"]["growthFactor"], 2
            ),
...
def write_stats_to_bigquery(json_row_list):
...
def write_to_bigquery(timeseries, metadata):
    ...
    if config.WRITE_BQ_STATS_FLAG:
        ...
        write_stats_to_bigquery(json_msg_list)
